...
void TEditor::stateTileSetOffset(){
	...
	for(auto cTile : mTileSet.TTiles){
						
		Tile* newTile = new Tile(); 
		newTile->loadFromBuffer(cTile->FileData, &mPalette);
		newTile->applyOffset(poffset, mGlobalSettings.bNewTileOffsetZero);
		TEActionReplaceTileSet* newAction = new TEActionReplaceTileSet();				
		newAction->doAction(cTile, newTile->FileData, this, &mTileSet);
      					
    	mTileSet.mActionStack.addAction(newAction);
	...
	showMessage("Color Offset Applied to TileSet");		
}
...
int TEditor::loadFromFolder(std::string path){
	...
	while(fs::exists(fs::status(cSprites))){
		...
		if(mGlobalSettings.getSpriteFileHeader(cSprites.string(), nSpriteX, nSpriteY, nSpriteBPP, sbuffer)){
			...
			cSKey = mGlobalSettings.mProjectSettings.getFrameScaleKey(nSpriteNum);
...
int TEditor::saveToFolder(std::string path){
	...
	if(mSprites.size()){
		for(int i = 0; i < mSprites.size(); i++){
			...
			if(cSKey){
				cSKey->ivalue = mSprites[i]->mMinFrameScale;
			} else {
				sKey *cNewSKey = mGlobalSettings.mProjectSettings.createFrameScaleKey(nSpriteNum);
				cNewSKey->ivalue = mSprites[i]->mMinFrameScale;
			}
	...
	while(fs::exists(fs::status(sSprite))){
		...
		cDelSKey = mGlobalSettings.mProjectSettings.getFrameScaleKey(nSpriteNum);

		if(cDelSKey){
			mGlobalSettings.mProjectSettings.removeFrameScaleKey(nSpriteNum);
		}
...
int TEditor::swapTiles(int source, int target, bool bDoCopy){
	if(mCurMode == EMODE_MAP){
		...
		} else if(bLShiftIsDown && bDoCopy){
			TEActionMoveTile* newAction = new TEActionMoveTile();				
